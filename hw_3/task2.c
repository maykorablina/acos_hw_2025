...
// This task only toucher realloc function, other comments are from task 1
#include "malloc.h"
...
void *realloc(void *ptr, size_t size) {
    ...
    if (asize < old_size) {
        ...
        if (rsize >= DSIZE) {
            ...
            PUT(new_hdr, rsize);
            PUT(PTR_ADD(new_hdr, rsize - WSIZE), rsize);
            coalesce(PTR_ADD(new_hdr, WSIZE));
        }
        g_heap.payload -= (old_size - DSIZE);
        g_heap.payload += size;
        return ptr;
    }

    // If the next block is empty and is sufficiently large, extend the current block instead of freeing it
    void *next_hdr = PTR_ADD(hdr, old_size);
    if (!GET_ALLOC(next_hdr)) {
        size_t next_size = GET_SIZE(next_hdr);
        size_t total_size = old_size + next_size;
        if (total_size >= asize) {
            PUT(hdr, total_size | 1);
            PUT(PTR_ADD(hdr, total_size - WSIZE), total_size | 1);
            ...
            if (remain >= DSIZE) {
                ...
                PUT(new_hdr, remain);
                PUT(PTR_ADD(new_hdr, remain - WSIZE), remain);
                coalesce(PTR_ADD(new_hdr, WSIZE));
            }

            g_heap.alloc += (asize - old_size);
            g_heap.payload += (size - (old_size - DSIZE));
            return ptr;
        }
    }
    return newptr;
